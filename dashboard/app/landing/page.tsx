    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; scroll-behavior: smooth; }
        ...
        .mnav-btn {
          display: flex; align-items: center; gap: 6px; padding: 7px 14px; border-radius: 999px; font-size: 13px; font-weight: 500; color: ${C.muted}; cursor: pointer; transition: all 0.2s; border: none; background: none; white-space: nowrap; text-decoration: none;
        }
        .mnav-btn:hover { color: ${C.text}; }
        .mnav-btn.active { background: rgba(124,58,237,0.18); color: ${C.violetL}; }
        .mnav-cta {
          margin-left: 4px; padding: 7px 16px; background: ${C.violet}; color: white; border-radius: 999px; font-size: 13px; font-weight: 600; cursor: pointer; border: none; text-decoration: none; transition: background 0.2s;
        }
        .mnav-cta:hover { background: ${C.violetL}; }

        /* ── Section base ── */
        .section { max-width: 1160px; margin: 0 auto; padding: 120px 24px; }
        .section-label {
          font-family: 'JetBrains Mono', monospace; font-size: 11px; font-weight: 500; letter-spacing: 2px; text-transform: uppercase; color: ${C.cyan}; margin-bottom: 16px; display: flex; align-items: center; gap: 8px;
        }
        .section-label::before { content: ''; display: inline-block; width: 20px; height: 1px; background: ${C.cyan}; }
        .section-title {
          font-family: 'Syne', sans-serif; font-size: clamp(32px, 5vw, 54px); font-weight: 800; letter-spacing: -1.5px; line-height: 1.05; color: ${C.text};
        }
        .section-title .hl-v { color: ${C.violetL}; }
        .section-title .hl-c { color: ${C.cyanL}; }

        /* ── Glass card ── */
        .glass-card { background: ${C.glass}; border: 1px solid ${C.border}; border-radius: 16px; backdrop-filter: blur(8px); }

        /* ── Hero ── */
        .hero-wrap {
          min-height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; padding: 140px 24px 80px; position: relative; overflow: hidden;
        }
        .hero-glow-v {
          position: absolute; top: -20%; left: 50%; transform: translateX(-50%); width: 900px; height: 500px; background: radial-gradient(ellipse, rgba(124,58,237,0.18) 0%, transparent 70%); pointer-events: none;
        }
        .hero-glow-c {
          position: absolute; bottom: 10%; right: -15%; width: 500px; height: 400px; background: radial-gradient(ellipse, rgba(6,182,212,0.10) 0%, transparent 70%); pointer-events: none;
        }
        .hero-eyebrow {
          font-family: 'JetBrains Mono', monospace; font-size: 12px; letter-spacing: 2px; text-transform: uppercase; color: ${C.cyan}; margin-bottom: 20px; display: flex; align-items: center; justify-content: center; gap: 10px;
        }
        .hero-badge {
          background: rgba(6,182,212,0.12); border: 1px solid rgba(6,182,212,0.3); border-radius: 999px; padding: 4px 12px; font-size: 11px; color: ${C.cyanL};
        }
        .hero-title {
          font-family: 'Syne', sans-serif; font-size: clamp(42px, 7vw, 80px); font-weight: 800; letter-spacing: -2.5px; line-height: 1.0; margin-bottom: 24px;
        }
        .hero-title .line1 { color: ${C.text}; display: block; }
        .hero-title .line2 {
          display: block; background: linear-gradient(135deg, ${C.violetL} 0%, ${C.cyanL} 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;
        }
        .hero-sub { font-size: 18px; color: ${C.muted}; max-width: 540px; line-height: 1.65; margin: 0 auto 48px; }
        .hero-sub strong { color: ${C.text}; font-weight: 600; }
        .hero-actions { display: flex; align-items: center; gap: 12px; flex-wrap: wrap; justify-content: center; margin-bottom: 80px; }
        .btn-primary {
          display: inline-flex; align-items: center; gap: 8px; padding: 13px 28px; background: linear-gradient(135deg, ${C.violet}, #5b21b6); color: white; border-radius: 10px; font-size: 15px; font-weight: 600; text-decoration: none; box-shadow: 0 8px 32px rgba(124,58,237,0.35); transition: all 0.2s; border: none; cursor: pointer;
        }
        .btn-primary:hover { transform: translateY(-2px); box-shadow: 0 12px 40px rgba(124,58,237,0.45); }
        .btn-ghost {
          display: inline-flex; align-items: center; gap: 8px; padding: 13px 24px; background: rgba(255,255,255,0.04); color: ${C.text}; border: 1px solid ${C.border}; border-radius: 10px; font-size: 15px; font-weight: 500; text-decoration: none; cursor: pointer; transition: all 0.2s;
        }
        .btn-ghost:hover { background: rgba(255,255,255,0.08); border-color: rgba(255,255,255,0.15); }

        /* ── Split HUD container ── */
        .hud-container { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; width: 100%; max-width: 900px; margin: 0 auto; }
        @media (max-width: 700px) { .hud-container { grid-template-columns: 1fr; } }

        .hud-panel { border-radius: 14px; overflow: hidden; border: 1px solid ${C.border}; background: rgba(14,14,20,0.95); }
        .hud-panel-header {
          display: flex; align-items: center; gap: 8px; padding: 10px 14px; background: rgba(255,255,255,0.03); border-bottom: 1px solid ${C.border}; font-family: 'JetBrains Mono', monospace; font-size: 11px; color: ${C.muted};
        }
        ...
        /* REC indicator */
        .rec-indicator {
          display: flex; align-items: center; gap: 8px; font-family: 'JetBrains Mono', monospace; font-size: 12px; font-weight: 500; color: ${C.red}; margin-bottom: 12px;
        }
        ...
        .rec-ring {
          position: absolute; inset: -4px; border-radius: 50%; border: 2px solid ${C.red}; animation: recRing 1.2s ease-out infinite;
        }
        @keyframes recRing { 0% { transform: scale(0.7); opacity: 0.9; } 100% { transform: scale(1.8); opacity: 0; } }

        .hud-stat-row { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 6px; margin-bottom: 10px; }
        .hud-stat { background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.06); border-radius: 8px; padding: 8px; text-align: center; }
        ...
        .hud-event {
          display: flex; align-items: center; gap: 8px; font-family: 'JetBrains Mono', monospace; font-size: 10px; color: ${C.muted}; padding: 4px 6px; border-radius: 5px; background: rgba(255,255,255,0.02);
        }
        .hud-event-type { font-size: 9px; font-weight: 700; padding: 1px 5px; border-radius: 3px; }

        /* Dashboard mini HUD */
        .dash-run-row {
          display: flex; align-items: center; justify-content: space-between; padding: 8px 10px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.06); background: rgba(255,255,255,0.025); margin-bottom: 6px; font-size: 12px;
        }
        .dash-run-name { color: ${C.text}; font-weight: 500; font-size: 11px; }
        .dash-run-meta { color: ${C.muted}; font-size: 10px; }
        .status-chip { font-size: 9px; font-weight: 700; padding: 2px 7px; border-radius: 20px; }
        ...
        .cp-card {
          border-radius: 12px; border: 1px solid ${C.border}; background: ${C.glass}; padding: 12px; cursor: pointer; transition: all 0.2s; position: relative; overflow: hidden;
        }
        ...
        .cp-thumb {
          height: 120px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 14px; display: flex; flex-direction: column; overflow: hidden; position: relative; background: #0f1117; box-shadow: 0 8px 16px rgba(0,0,0,0.5);
        }
        .mini-dlg-header {
          height: 20px; background: #1a1d27; border-bottom: 1px solid rgba(255,255,255,0.1); display: flex; align-items: center; justify-content: space-between; padding: 0 6px;
        }
        ...
        .mini-row {
          display: flex; gap: 4px; padding: 4px 6px; border-bottom: 1px solid rgba(255,255,255,0.03); align-items: center; font-family: 'JetBrains Mono', monospace; font-size: 7px; color: #e5e7eb;
        }
        ...
        .cp-num { font-family: 'JetBrains Mono', monospace; font-size: 10px; font-weight: 600; color: var(--cp-color); text-transform: uppercase; }
        .cp-label { font-size: 15px; font-weight: 700; color: ${C.text}; letter-spacing: -0.3px; }

        .cp-detail {
          border-radius: 14px; border: 1px solid ${C.border}; background: rgba(14,14,20,0.98); padding: 28px; display: grid; grid-template-columns: 1fr 1fr; gap: 24px; min-height: 240px;
        }
        @media (max-width: 700px) { .cp-detail { grid-template-columns: 1fr; } }
        .cp-detail-title {
          font-family: 'JetBrains Mono', monospace; font-size: 10px; color: ${C.muted}; text-transform: uppercase; letter-spacing: 1.5px; margin-bottom: 10px; display: flex; align-items: center; gap: 6px;
        }
        .cp-log-line {
          font-family: 'JetBrains Mono', monospace; font-size: 11px; padding: 5px 8px; border-radius: 5px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); margin-bottom: 4px;
        }
        .log-info { color: ${C.cyan}; }
        .log-warn { color: ${C.yellow}; }
        .log-debug { color: ${C.muted}; }
        .net-badge { display: inline-flex; align-items: center; gap: 6px; font-family: 'JetBrains Mono', monospace; font-size: 11px; padding: 5px 10px; border-radius: 6px; margin-bottom: 8px; }
        ...
        .net-payload {
          font-family: 'JetBrains Mono', monospace; font-size: 10px; color: '#9ca3af'; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); border-radius: 7px; padding: 10px; line-height: 1.5; color: #9ca3af;
        }
        ...
        .flow-vis { position: relative; height: 280px; display: flex; align-items: center; justify-content: center; }
        .flow-node {
          position: absolute; display: flex; flex-direction: column; align-items: center; gap: 8px; font-family: 'JetBrains Mono', monospace; font-size: 11px; color: ${C.muted};
        }
        .flow-node-icon { width: 52px; height: 52px; border-radius: 12px; display: flex; align-items: center; justify-content: center; }
        .flow-line {
          position: absolute; top: 50%; left: 23%; width: 54%; height: 1px; background: linear-gradient(90deg, rgba(124,58,237,0.4), rgba(6,182,212,0.4));
        }
        .flow-dot-track {
          position: absolute; top: calc(50% - 6px); left: 23%; width: 54%; height: 12px; overflow: visible; pointer-events: none;
        }

        .queue-card { border-radius: 12px; border: 1px solid ${C.border}; background: rgba(14,14,20,0.9); overflow: hidden; }
        .queue-header {
          padding: 14px 18px; border-bottom: 1px solid ${C.border}; font-size: 13px; font-weight: 600; display: flex; align-items: center; justify-content: space-between;
        }
        .queue-item {
          display: flex; align-items: center; justify-content: space-between; padding: 10px 18px; border-bottom: 1px solid rgba(255,255,255,0.04); font-size: 12px; transition: background 0.15s; cursor: default;
        }
        .queue-item:hover { background: rgba(255,255,255,0.02); }
        .queue-item:last-child { border-bottom: none; }
        .queue-item-left { display: flex; align-items: center; gap: 10px; }
        .queue-order {
          width: 22px; height: 22px; border-radius: 50%; background: rgba(255,255,255,0.06); display: flex; align-items: center; justify-content: center; font-size: 10px; font-weight: 700; color: ${C.muted};
        }
        ...
        .persona-card {
          padding: 28px; border-radius: 16px; border: 1px solid ${C.border}; background: ${C.glass}; transition: all 0.25s; cursor: default;
        }
        .persona-card:hover {
          border-color: var(--persona-accent); background: var(--persona-glow); transform: translateY(-3px); box-shadow: 0 16px 48px rgba(0,0,0,0.3);
        }
        .persona-icon-wrap { width: 44px; height: 44px; border-radius: 10px; display: flex; align-items: center; justify-content: center; margin-bottom: 16px; }
        .persona-role {
          font-family: 'JetBrains Mono', monospace; font-size: 10px; text-transform: uppercase; letter-spacing: 1.5px; color: var(--persona-accent); margin-bottom: 8px;
        }
        .persona-headline {
          font-family: 'Syne', sans-serif; font-size: 19px; font-weight: 700; letter-spacing: -0.4px; color: ${C.text}; margin-bottom: 10px;
        }
        ...
        .cta-glow {
          position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 600px; height: 400px; background: radial-gradient(ellipse, rgba(124,58,237,0.15), transparent 70%); pointer-events: none;
        }
        .cta-title {
          font-family: 'Syne', sans-serif; font-size: clamp(36px, 5vw, 60px); font-weight: 800; letter-spacing: -1.5px; margin-bottom: 20px; position: relative;
        }
        .cta-sub { font-size: 17px; color: ${C.muted}; max-width: 420px; margin: 0 auto 40px; line-height: 1.65; }

        /* ── Footer ── */
        .lp-footer { border-top: 1px solid ${C.border}; padding: 32px 24px; text-align: center; font-size: 13px; color: ${C.muted}; }
      ...
      <div className="lp" ref={containerRef}>
        ...
        <section id="hero">
          <motion.div className="hero-wrap" style={{ y: heroBgY }}>
            ...
            >
              <a href={CHROME_EXTENSION_URL} target="_blank" rel="noopener noreferrer" className="btn-primary">
                Install Extension
              </a>
